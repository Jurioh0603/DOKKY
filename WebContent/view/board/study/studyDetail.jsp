...
<link rel="stylesheet" href="../../../css/dokkyCss/studyDetailStyle.css"> <!-- 스타일 시트 링크 -->
...
<br>
   ...
   <div class="comment-form">
		<form id="addReplyForm" action="/study/reply.do" method="post">
...
<script>
	...
	document.getElementById("addReplyButton").addEventListener(
			"click",
			function() {
				// 댓글 등록 폼 가져오기
				...
				// 댓글 내용 가져오기
				var replyContent = addReplyForm
						.querySelector("textarea[name='rcontent']").value
						.trim();
	...
	document.addEventListener('click', function(event) {
		...
		if (event.target.classList.contains('edit-reply-btn')) {
			...
			//원래 댓글 내용 가져오기
			var originalContent = event.target.closest('.comment-item')
					.querySelector('.comment-content').textContent.trim();
	...
	document.addEventListener('click', function(event) {
		...
		if (event.target.classList.contains('delete-reply-btn')) {
			...
			if (confirm('댓글을 삭제하시겠습니까?')) {
				...
				var form = document.createElement('form');
				form.method = 'POST';
				form.action = '/study/reply.do?command=removeReply';
